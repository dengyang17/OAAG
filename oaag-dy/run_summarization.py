...
# Where to find data
tf.app.flags.DEFINE_string('data_path', 'data/sport/chunked/train_*', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('valid_path', 'data/sport/chunked/test_*', 'Path expression to tf.Example validation datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('vocab_path', 'data/sport/vocab', 'Path expression to text vocabulary file.')
...
tf.app.flags.DEFINE_string('log_root', 'model', 'Root directory for all logging.')
...
tf.app.flags.DEFINE_integer('review_num', 10, 'max number of reviews for each question.')
tf.app.flags.DEFINE_integer('beam_size', 10, 'beam size for beam search decoding.')
tf.app.flags.DEFINE_integer('min_dec_steps', 30, 'Minimum sequence length of generated summary. Applies only for beam search decoding mode')
...
tf.app.flags.DEFINE_boolean('glove', True, 'glove pre-trained embeddings')
...
tf.app.flags.DEFINE_float('sa_loss_wt', 0.2, 'Weight of opinion mining loss (lambda in the paper). If zero, then no incentive to minimize coverage loss.')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer, vocab, hps):
...
def run_eval(model, sess, val_batcher):
  loss = []
  y_pred = []
  y_true = []
