...
def attention_decoder(decoder_inputs, initial_state, question_states, review_states, opinion_states, num, q_padding_mask, r_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, q_prev_coverage=None, r_prev_coverage=None):
  """
  Args:
    ...
    opinion_states: 3D Tensor [batch_size x revier_num x attn_size]
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    o_attn_size = opinion_states.get_shape()[2].value
    ...
    opinion_states = tf.expand_dims(opinion_states, axis=2)
    ...
    W_o = variable_scope.get_variable("W_o", [1, 1, o_attn_size, attention_vec_size])
    opinion_features = nn_ops.conv2d(opinion_states, W_o, [1, 1, 1, 1], "SAME")  # shape (batch_size,review_num*attn_length,1,attention_vec_size)

    # Get the weight vectors v and w_c (w_c is for coverage)
    v_q = variable_scope.get_variable("v_q", [attention_vec_size])
    v_r = variable_scope.get_variable("v_r", [attention_vec_size])
    v_o = variable_scope.get_variable("v_o", [attention_vec_size])
    if use_coverage:
    ...
    def op_attention(decoder_features):
      with variable_scope.variable_scope("Opinion_Attention"):
        # Calculate v^T tanh(W_h h_i + W_s s_t + w_c c_i^t + b_attn)
        e = math_ops.reduce_sum(v_o * math_ops.tanh(opinion_features + decoder_features), [2, 3])  # shape (batch_size,attn_length)
        sent_attention = nn_ops.softmax(e)
      return sent_attention


    def q_attention(decoder_features, coverage=None):
    ...
    def r_attention(decoder_features, sent_attention, coverage=None):
      """Calculate the context vector and attention distribution from the decoder state.
    ...
    if initial_state_attention: # true in decode mode
      ...
      sent_attention = op_attention(decoder_features)
      q_context_vector, _, q_coverage = q_attention(decoder_features, q_coverage) # in decode mode, this is what updates the coverage vector
      r_context_vector, _, r_coverage = r_attention(decoder_features, sent_attention, r_coverage)
    for i, inp in enumerate(decoder_inputs):
      ...
      if i == 0 and initial_state_attention:  # always true in decode mode
        with variable_scope.variable_scope(variable_scope.get_variable_scope(), reuse=True): # you need this because you've already run the initial attention(...) call
          with variable_scope.variable_scope('Decoder'):
            decoder_features = linear(state, attention_vec_size, True) # shape (batch_size, attention_vec_size)
            decoder_features = tf.expand_dims(tf.expand_dims(decoder_features, 1), 1) # reshape to (batch_size, 1, 1, attention_vec_size)
          sent_attention = op_attention(decoder_features)
          q_context_vector, q_attn_dist, _ = q_attention(decoder_features, q_coverage) # don't allow coverage to update
          r_context_vector, r_attn_dist, _ = r_attention(decoder_features, sent_attention, r_coverage)
      else:
        with variable_scope.variable_scope('Decoder'): # you need this because you've already run the initial attention(...) call
          decoder_features = linear(state, attention_vec_size, True) # shape (batch_size, attention_vec_size)
          decoder_features = tf.expand_dims(tf.expand_dims(decoder_features, 1), 1) # reshape to (batch_size, 1, 1, attention_vec_size)
        sent_attention = op_attention(decoder_features)
        q_context_vector, q_attn_dist, q_coverage = q_attention(decoder_features, q_coverage)
        r_context_vector, r_attn_dist, r_coverage = r_attention(decoder_features, sent_attention, r_coverage)
