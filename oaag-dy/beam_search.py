...
class Hypothesis(object):
  ...
  def __init__(self, tokens, log_probs, state, q_attn_dists, r_attn_dists, p_gens, q_coverage, r_coverage):
  ...
  def extend(self, token, log_prob, state, q_attn_dist, r_attn_dist, p_gen, q_coverage, r_coverage):
    ...
    '''if len(self.tokens) > 3:
                  trigram = [self.tokens[i:i+3] for i in range(len(self.tokens)-2)]
                  if self.tokens[-2:]+[token] in trigram:
                    log_prob = min(min(self.log_probs), log_prob)'''
